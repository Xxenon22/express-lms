...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        ...
        if (req.originalUrl.includes("/pg")) type = "pg";
        else if (req.originalUrl.includes("/essai")) type = "essai";
        else if (req.originalUrl.includes("/soal")) {
            // CREATE ASSIGNMENT
            type = file.fieldname.startsWith("pg_")
                ? "pg"
                : file.fieldname.startsWith("essai_")
                    ? "essai"
                    : "temp";
        }
