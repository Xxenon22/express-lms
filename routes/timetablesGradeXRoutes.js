...
const storage = multer.diskStorage({
    destination: function (req, file, cb) {
        cb(null, "uploads/timetables-grade-x"); // folder penyimpanan
    },
...
router.post("/", upload.single("jadwal"), async (req, res) => {
    try {
        ...
        const result = await pool.query(
            "INSERT INTO jadwal_x_db (file_name) VALUES ($1) RETURNING *",
            [file_name]
        );
...
router.get("/", async (req, res) => {
    try {
        const result = await pool.query("SELECT * FROM jadwal_x_db ORDER BY id DESC");
...
router.delete("/:id", async (req, res) => {
    try {
        ...
        // hapus dari db
        await pool.query("DELETE FROM jadwal_x_db WHERE id = $1", [id]);

        // hapus juga file fisiknya
        import("fs").then(fs => {
            const filePath = path.join("uploads/timetables-grade-x", fileName);
