...
router.post("/", upload.single("jadwal"), async (req, res) => {
    try {
        ...
        const result = await pool.query(
            `INSERT INTO jadwal_x_db (file_name, file_data, file_mime)
             VALUES ($1, $2, $3)
             RETURNING id`,
...
router.get("/", async (req, res) => {
    const result = await pool.query(
        "SELECT id, file_name FROM jadwal_x_db ORDER BY id DESC"
    );
...
router.get("/:id/file", async (req, res) => {
    ...
    const result = await pool.query(
        "SELECT file_data, file_mime, file_name FROM jadwal_x_db WHERE id = $1",
        [id]
    );
